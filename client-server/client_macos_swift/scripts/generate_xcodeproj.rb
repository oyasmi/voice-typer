...
PROJECT_PATH = File.join(ROOT, 'VoiceTyperClient.xcodeproj')
TARGET_NAME = 'VoiceTyperClient'
PRODUCT_BUNDLE_IDENTIFIER = 'com.voicetyper.client'
...
# 2.7.0：与既有 .xcodeproj（此前唯一权威来源）保持一致；此脚本原先写的 2.6.1 是过期值，
# 与 pbxproj 实际生效的版本号不一致，这里一并修正。版本号本身按用户要求保持不变。
project.build_configuration_list.set_setting('MARKETING_VERSION', '2.7.0')
...
app_target.build_configurations.each do |config|
  ...
  settings['MARKETING_VERSION'] = '2.7.0'
