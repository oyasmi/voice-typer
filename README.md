...
- **松手就是完成。** 最终识别会自动补上标点，并将口述数字整理为更自然的书写形式，例如把
  “六十四兆”写成“64兆”。
- **它不常驻吃内存。** macOS 版默认在首次按下热键时才把识别引擎载入内存（与录音并行，约一秒），
  空闲一段时间后再自动释放；开机自启也不会为一次可能不会发生的听写常驻数百 MB。
...
自动下载、校验并加载约 240MB 的语音模型，无需手动点击下载；模型只需下载一次。

macOS 版把这段准备做成了四步引导（欢迎 → 权限 → 模型 → 试一试），最后一步会让你在引导窗口里
真正完成一次听写：权限全部授予并不等于能用，只有把「热键 → 麦克风 → 识别 → 插入」整条链跑通
才算数，哪一环出问题都会当场指出来。
...
短于 0.3 秒的按键会被当作误触丢弃。按 `Esc` 可以取消本次听写，临时改主意时不会留下任何文字
（macOS 在录音中与识别中都可取消，Windows 目前限录音中）。macOS 还可以把触发方式改成
「按一次开始、再按一次结束」，长段口述时手指不必一直按住；浮窗位置也可以改成右下角、
跟随光标或干脆不显示。

macOS 版的浮窗在录音时会显示当前输入设备名，并在持续采不到声音时主动提示检查麦克风——
不必等说完才发现结果是空的。
...
| 中英双语界面 | 菜单、设置窗口与浮窗都有对等的中英文案，默认中文，可在设置里切换（重启生效） |
...
3. 第一次打开时，跟着应用内的四步引导授予麦克风、辅助功能和输入监控权限。
4. 应用会同时自动准备语音模型；在引导最后一步试一次听写，确认整条链路可用。
...
一体化应用目前专注于 SenseVoice-Small，不支持更换为 paraformer。确有需要时，可以改用
客户端—服务端版本并在服务端切换模型。热词则在两种形态里都已移除：早期版本有过该功能，
但在当前的识别链路上从未真正生效。清晰、稳定的默认体验优先于暴露大量尚未打磨的选项。
