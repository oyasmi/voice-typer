...
| `macos/`（新，主分发版本） | **`VoiceTyper`** | **`com.voicetyper.app`** | **3.2.1**（当前版本，见 D3） |
...
| # | 改动 | 实测/量级依据 |
| --- | --- | --- |
| R6-06 | **默认不在启动时加载模型**（`asr.preload_on_launch`，默认 false） | 旧行为是启动即加载、`idle_unload_minutes` 到点再卸载：开机自启的用户每次开机都白花一次 0.85s 加载 + 十分钟 ~510MB，而当天可能一次都没用。关掉后首次按热键才加载，且加载与录音并行（`makeSession`），用户通常正在说第一句话。新增 `ASRService.prepareWithoutLoading()` 只定位模型文件并置为 `.suspendedForIdle`——那个状态的语义本来就是"就绪但引擎不在内存里" |
...
| D9 | 启动时是否预加载模型 | ✅ **默认否**（`asr.preload_on_launch = false`） | 旧的"启动即加载、10 分钟后卸载"让开机自启用户每天白付一次加载与十分钟 510MB；按需加载与录音并行，代价接近于零（§11.3 R6-06） |
