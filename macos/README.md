...
[← 返回主项目](../README.md) · [设计方案](DESIGN.md) · [分体式客户端](../client_macos_swift/README.md)

单进程 macOS 菜单栏应用：把 [`server/`](../server/README.md) 的 SenseVoice 识别链路用 Swift 重写并
内联进客户端，拖进「应用程序」打开即用，**不需要**单独部署 Python 服务端。当前版本 **3.0.0**，
...
- 只支持 SenseVoice-Small 模型，不支持 paraformer / 热词（如需这些能力，用
  [分体式客户端](../client_macos_swift/README.md) + [服务端](../server/README.md)）
...
若之前装过 [`client_macos_swift/` 的 VoiceTyperClient](../client_macos_swift/README.md)，
...
sha256，支持断点续传。如果这台机器之前跑过 [`server/`](../server/README.md)（`~/.cache/modelscope/`
...
`server/voice_typer_server/recognizer.py` 的 Swift 移植，已用金标准测试逐点验证。
...
| 测试 | 内容 |
| --- | --- |
| `FbankParityTests` | fbank / LFR+CMVN 特征逐点比对 `server/` 产出的金标准（阈值 1e-3） |
...
即可），缺失时自动 `XCTSkip`。夹具由 `scripts/dump_reference_fixtures.py` 生成（需要 `server/`
的 Python 环境）。
...
参见[分体式客户端文档的对应章节](../client_macos_swift/README.md#热键完全没反应)——`CGEventTap`
实现完全一致，排查方式相同（换成本应用的 Bundle ID `com.voicetyper.app`）。

### 识别成功但文字没插进去

同上，参见[分体式客户端文档](../client_macos_swift/README.md#识别成功但文字没插进去)，文本插入
实现代码原样搬运，行为一致。
...
- [设计方案](DESIGN.md)
- [分体式客户端（多设备共享服务端场景）](../client_macos_swift/README.md)
