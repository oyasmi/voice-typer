...
- ⚡ **流式实时预览** - 默认流式模式，识别结果边说边显示在 HUD 浮窗中；预览与最终文本出自同一个 SenseVoice 模型，会自我修正且松手时不跳变
...
3. 流式模式下，识别片段会实时显示在 HUD 浮窗中
4. **松开** 自动完成识别并将最终文本插入当前光标位置
5. 录音过程中按 **Esc** 可随时取消本次录音，不触发识别与插入
