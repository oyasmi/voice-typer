...
final class StatusBarController: NSObject {
    ...
    private let toggleMenuItem = NSMenuItem(title: "暂停", action: #selector(handleToggle), keyEquivalent: "")
    ...
    override init() {
        ...
        [toggleMenuItem, setupMenuItem, reconnectMenuItem, configMenuItem, quitMenuItem].forEach {
            $0.target = self
        }
    ...
    func update(state: AppState, hotkeyDisplay: String, serverStatus: String, isEnabled: Bool) {
        ...
        toggleMenuItem.title = isEnabled ? "暂停" : "启用"
    }
