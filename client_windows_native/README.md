...
- **流式识别（默认）**：WebSocket 实时回传，HUD 上同步显示预览；服务端默认用 SenseVoice 单模型同时产出预览与最终文本，预览会自我修正（每帧为全量文本，直接替换），松手时措辞不会跳变。
...
服务端默认就是流式模式（`v1.2.0+`），本客户端勾选"流式识别"即可。如果你的服务端启动时显式加了 `--no-streaming`，请在客户端设置中取消"流式识别"以匹配。详见 [服务端文档](../server/README.md)。
...
录音不足 0.3 秒不会触发识别（客户端本地过滤，不会发出请求；服务端不做强制校验）。
